...
export default function ChatLayout() {
  ...
  useEffect(() => {
    if (userId) {
      dispatch(fetchMessagesThunk(String(userId)))
        .unwrap()
        .then((data) => {
          // Utilise la conversationId renvoyée par le thunk
          dispatch(setCurrentConversation(data.conversationId));
        });
    }
  }, [userId, dispatch]);
  ...
  const handleSendMessage = async () => {
    ...
    // on snapshotte la valeur avant de vider (pour éviter un état “bloqué” visuel)
    const content = message.trim();
    setMessage(""); // le champ se vide tout de suite

    // dispatch optimiste (le message apparaît immédiatement)
    dispatch(
      sendMessageThunk({
        content,
  ...
  console.log({
    ...
    usersIds: users.map((u) => typeof u.user_id + ":" + u.user_id),
    selectedUser,
  });
